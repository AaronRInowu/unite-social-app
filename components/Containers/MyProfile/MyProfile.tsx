...
export const MyProfileScreen = ({ me, questionIds }: ddd) => {
  const [notifState, setNotifState] = useState(false);
  const selectedUser = me;
  const userAnswers = selectedUser?.answers?.docs
    ? selectedUser.answers.docs.map((m) => (typeof m === "number" ? m : m.id))
    : [];
  const allPromptsFilled = !questionIds.some((q) => !userAnswers.includes(q));
        ...
        <View className="flex-row gap-3 justify-evenly py-4">
          <View className="items-center gap-3">
            ...
            <Text className="text-white text-3xl">
              {selectedUser.connections?.docs
                ? selectedUser.connections.docs.length
                : 0}
            </Text>
          </View>
          <View className="items-center gap-3">
